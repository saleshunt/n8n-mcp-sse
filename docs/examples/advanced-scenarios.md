...
## Building AI Agent Workflows Safely

With the capabilities registry and strict validation, you can wire specialized ports:

```javascript
await useWorkflowCreate({
  name: "AI Categorization Subflow",
  nodes: [
    /* ... nodes including AI Agent, OpenAI Chat Model, Structured Output Parser ... */
  ],
  edges: [
    {
...
If an edge uses an unsupported port for a node, the server throws a detailed validation error before calling n8n.
...
const stats = await accessMcpResource(
  "n8n-mcp-server",
  "n8n://execution-stats"
);
...
for (const workflow of workflows) {
  ...
  if (nodeCount > 15) {
    suggestions.push(
      "Consider splitting into multiple workflows for better maintainability"
    );
  }
  ...
  if (avgExecutionTime > 30) {
    suggestions.push(
      "Long execution time - check for bottlenecks or inefficient operations"
    );
  }

  if (
    nodeTypes.has("n8n-nodes-base.httpRequest") &&
    !nodeTypes.has("n8n-nodes-base.errorTrigger")
  ) {
    suggestions.push(
      "HTTP requests without error handling - consider adding error triggers"
    );
  }
...
for (const workflow of workflows) {
  ...
  const scheduleNodes = details.nodes.filter(
    (node) =>
...
const workflows = await useMcpTool("n8n-mcp-server", "workflow_list", {});
const existingMonitor = workflows.find(
  (workflow) =>
...
const legacyWorkflow = workflows.find((w) =>
  w.name.toLowerCase().includes("legacy crm")
);
const newWorkflow = workflows.find((w) =>
  w.name.toLowerCase().includes("new crm")
);
...
const migrationWorkflow = await useMcpTool(
  "n8n-mcp-server",
  "workflow_create",
  {
